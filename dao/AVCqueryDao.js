...
exports.queryKics = async function () {
    try {
        ...
        // avl_calc
        connection.query(
            `INSERT INTO avl_calc 
            (SETL_YM, EXE_IDNO, PAP_NAV, DISAPP_CAPITAL,SUGI1
			, SUGI2, REFUND_DEFICIT, REFUND_RSV, REFUND_RSV_EXS, SUGI8
			, SUGI9, SUGI15, ADD_AVL_CAPITAL
            )
			...
			, 30261
            , (SELECT SUGI17+SUGI18+SUGI19+SUGI20+SUGI21+SUGI22+SUGI23
            FROM SUGI_DT
			...
			, (SELECT GREATEST((5 - (SELECT SUGI6+SUGI7
            FROM SUGI_DT
			WHERE SETL_YM = (SELECT SETL_YM FROM BASE_YM)
			AND EXE_IDNO = (SELECT EXE_IDNO FROM BASE_YM)
            ))*(1-(SELECT SUGI16/100
		   ...
		   , (SELECT SUGI10+SUGI11+LEAST(SUGI12,SUGI13)+SUGI14
			FROM SUGI_DT
			WHERE SETL_YM = (SELECT SETL_YM FROM BASE_YM)
			AND EXE_IDNO = (SELECT EXE_IDNO FROM BASE_YM)
            )
